...
点名曲目后那段短评由 AstrBot 当前启用的大模型生成，默认不超过 100 字。短评会先检查投稿版本、题材风险和检索可信度，但不会为了显得专业而硬挑缺点：作品好听时可以直接夸，敏感题材则会说清楚，同时不会把题材特殊直接等同于音乐质量差。翻唱、二创、AI 版本、争议或可能引起不适的内容仍会明确说明；拿正常单曲时长强行批评“展开仓促”、或者一口气堆叠多个空泛缺点的短评会被自动退回重写。短评可以通过 `review.enabled` 关闭，长度由 `review.max_chars` 控制。

## 每日推送

在目标群里执行一次 `/jrsq bind` 就算绑定好了。默认每天 `12:00` 推送，时区是 `Asia/Shanghai`。机器人会依次发送“今日推荐：曲名”、自己的短评和视频，不把三样东西挤成一条消息。最近 14 次推过的曲目会暂时避开；曲库太小时才会重新抽到旧曲。
...
点歌可以说得随意一点，比如 `/jrsq 想听你说月色真美`。插件会保留原句，同时拆出更像曲名的部分，用普通检索和“原曲 / 本家”检索一起召回候选，全部收齐后再判断，不会因为第一条标题很像就提前收工。遇到中、日、英标题不同的曲子时，它会从可靠的双语结果继续找一次正式曲名，但推导出的别名不会和用户亲口说的曲名拿同等权重。排序还会综合作者、来源署名、跨检索重复出现、标签、时长和本家标记；演唱会、MMD、翻唱、手书、谱面、短片、补档和标题党投稿会被过滤或降权。已经知道投稿的话，直接发 BV 号或带 BV 号的链接最稳。

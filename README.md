...
点名曲目后那段短评由 AstrBot 当前启用的大模型生成，默认不超过 100 字。它可以通过 `review.enabled` 关闭，长度则由 `review.max_chars` 控制。随机推荐和每日推送不会额外发短评。
...
插件默认不要求登录 B站。它会先建立一个匿名访问会话再搜索；接口仍不可用时，才依次改走搜索网页和 yt-dlp。搜到结果后还会从前几条里辨认可能的正式曲名，处理常见译名差异和记错一两个字的情况，再按曲名、作者、标签和投稿类型重排，尽量避开翻唱、教程、手书、音游谱面、翻跳和游戏版 MV。已经知道投稿的话，直接发 BV 号或带 BV 号的链接最稳。

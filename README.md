...
点名曲目后那段短评由 AstrBot 当前启用的大模型生成，默认不超过 100 字。短评会先检查投稿版本、题材风险和检索可信度，但不会为了显得专业而硬挑缺点：作品好听时可以直接夸，敏感题材则会说清楚，同时不会把题材特殊直接等同于音乐质量差。翻唱、二创、AI 版本、争议或可能引起不适的内容仍会明确说明；拿正常单曲时长强行批评“展开仓促”、或者一口气堆叠多个空泛缺点的短评会被自动退回重写。短评可以通过 `review.enabled` 关闭，长度由 `review.max_chars` 控制。随机推荐和每日推送不会额外发短评。
